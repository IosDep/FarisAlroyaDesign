...
class VideoPlayerController: NSObject, NSCacheDelegate {
    ...
    func isCurrentlyPlaying(url: String) -> Bool {
          return self.videoURL == url && self.currentLayer?.player?.rate != 0
      }

    func isVideoContainerPlaying(url: String) -> Bool {
          guard let videoContainer = videoCache.object(forKey: url as NSString) else {
              return false
          }
          return videoContainer.playOn
      }
    func pauseCurrentVideo() {
            if let url = videoURL, let layer = currentLayer {
    ...
    func playVideo(withLayer layer: AVPlayerLayer, url: String, volume: Float = 1.0) {
        videoURL = url
        currentLayer = layer
        if let videoContainer = self.videoCache.object(forKey: url as NSString) {
            ...
            videoContainer.player.volume = volume // Set the volume
     ...
     */
    func pausePlayeVideosFor(tableView: UITableView, appEnteredFromBackground: Bool = false,flag:Int  = 0) {
